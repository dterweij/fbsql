...
/* MySQL 4 Code Revisions */
/* Copyright 2003, Luke Blackwell. http://www.sos-technology.com mailto:lblackwell@sos-technology.com */
...
int fbsql_connect(Tcl_Interp *interp, int sql_number, int argc, char **argv) {
	...
	char *user = NULL;
	char *passwd = NULL;
	char *db = NULL; //this line can be removed if you want to user method 1 or 2
	...
	if (argc > 3 && argv[3]) { db = argv[3]; } //this line can be removed if you want to user method 1 or 2

	mysql_init(&connection[sql_number].mysql);
	
	//1. hard coded connect line
	//if (!(mysql_real_connect(&connection[sql_number].mysql, host, user, passwd, "databasename", 0, NULL, 0))) {	
	
	//2. connect and still use sql selectdb $databse
	//if (!(mysql_real_connect(&connection[sql_number].mysql, host, user, passwd, NULL, 0, NULL, 0))) {	
		
	//3. connect in the format mysql connect $host $user $passwd $db
  if (!(mysql_real_connect(&connection[sql_number].mysql, host, user, passwd, db, 0, NULL, 0))) {	
